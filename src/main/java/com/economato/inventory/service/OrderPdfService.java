    ...
    private void addProductsTable(Document document, List<OrderDetailResponseDTO> details,
        ...
        if (details != null) {
            for (int i = 0; i < details.size(); i++) {
                ...
                table.addCell(
                        createTableDataCell(formatDecimal(detail.getQuantity()) + " uds", regularFont, rowBg, false));
                table.addCell(createTableDataCell(formatCurrency(detail.getUnitPrice()), regularFont, rowBg, false));
                table.addCell(createTableDataCellAccent(formatCurrency(detail.getSubtotal()), boldFont, rowBg));
            }
        }
